#!/usr/bin/env python3
"""PoC v17: Bucketed tile clustering + bitmap compression analysis.

Tests:
  1. Sort tiles by difficulty, group into N buckets, assign tier per bucket
  2. Measure bitmap entropy with different bucket sizes
  3. Compare bitmap storage: 2-bit per tile vs bucketed approach
  4. Test if run-length encoding helps (tiles sorted by tier)
"""
...
def run_length_encode(tier):
    """Estimate RLE cost for a tier assignment."""
    if not tier: return 0
    runs = 1
    for i in range(1, len(tier)):
        if tier[i] != tier[i-1]:
            runs += 1
    # Each run: 2 bits (tier) + log2(n_tiles) bits (run length)
    import math
    run_len_bits = max(1, math.ceil(math.log2(len(tier))))
    return runs * (2 + run_len_bits) / len(tier)
...
def run_experiment(data_dir, device, ext, ghd, tcp, tcpi, qtf, cbs):
    ...
    for ei in range(n_experts):
        ...
        # For each target bpw, compute tier assignment and analyze bitmap
        print(f"\n  Expert {ei} bitmap analysis:", flush=True)
        ...
        for target_bpw in [3.5, 4.0, 4.5, 5.0, 5.5]:
            ...
            for i in range(n_tiles):
                upgrades.append((imp34[i].item(), i, 3, 4))
                upgrades.append((imp45[i].item(), i, 4, 5))
                upgrades.append((imp56[i].item(), i, 5, 6))
            ...
            # 2-bit per tile cost
            cost_2bit = 2.0  # bits per tile
            
            # Entropy-coded cost
            cost_entropy = entropy  # bits per tile
            
            # RLE cost (sorted by tier)
            tier_sorted = sorted(tier)
            rle_cost = run_length_encode(tier_sorted)
            
            # RLE cost (original order — no spatial structure)
            rle_cost_unsorted = run_length_encode(tier)
            
            # Convert to bpw overhead
            bpw_2bit = 2.0 * n_tiles / (k * n)
            bpw_entropy = entropy * n_tiles / (k * n)
            bpw_rle = rle_cost * n_tiles / (k * n)
            bpw_rle_unsorted = rle_cost_unsorted * n_tiles / (k * n)
            
            print(f"    {target_bpw:.1f}bpw: tiers={dict(counts)} H={entropy:.3f} "
                  f"2bit={bpw_2bit:.6f} entropy={bpw_entropy:.6f} "
                  f"RLE_sorted={bpw_rle:.6f} RLE_unsorted={bpw_rle_unsorted:.6f}", flush=True)
...
def main():
    ...
    ap.add_argument("--out", default="/tmp/poc_residual/results_v17.json")
    args = ap.parse_args()
