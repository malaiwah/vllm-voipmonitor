#!/usr/bin/env python3
"""PoC v37: Complete hybrid Pareto with all rescaled trellis tiers.

Tests ALL base + rescaled-trellis combinations to find the best at each bpw:
- K2+K1trsc (3), K2+K2trsc (4), K2+K3trsc (5), K2+K4trsc (6), K2+K5trsc (7)
- K3+K1trsc (4), K3+K2trsc (5), K3+K3trsc (6), K3+K4trsc (7), K3+K5trsc (8)
- K4+K1trsc (5), K4+K2trsc (6), K4+K3trsc (7), K4+K4trsc (8), K4+K5trsc (9)

Plus LM tiers for 8-10 bpw.
Builds definitive hybrid Pareto with 10 experts.
"""
...
def run_experiment(data_dir, device, ext, ghd, tcp, tcpi, qtf, cbs_scale):
    ...
    for ei in range(n_experts):
        ...
        # Rescaled trellis on residual: all base × all K_res
        for base_q, bn, res, bb in [(qk2, "K2", r2, 2.0), (qk3, "K3", r3, 3.0), (qk4, "K4", r4, 4.0)]:
            for Kr in [1, 2, 3, 4, 5]:
                recon = rescaled_trellis(base_q, res, Kr, device, tcp, tcpi, qtf, cbs_scale)
                tiers[f"{bn}+K{Kr}trsc"] = (recon, bb + Kr)

        # LM tiers for 8-10 bpw
        for base_q, bn, res, bb in [(qk2, "K2", r2, 2.0), (qk3, "K3", r3, 3.0), (qk4, "K4", r4, 4.0)]:
    ...
    print(f"\n  Definitive hybrid Pareto (10 experts):", flush=True)
    pareto = []
    for target_10 in range(20, 101, 2):
    ...
    results["all_tiers"] = {name: {"bpw": d["bpw"], "avg_mse": avg_tmse[name].mean().item()}
                            for name, d in tier_data.items()}
    return results
...
def main():
    ...
    ap.add_argument("--out", default="/tmp/poc_residual/results_v37.json")
    args = ap.parse_args()
