#!/usr/bin/env python3
"""PoC v16: Extend Pareto frontier to 6.0 bpw with K5 + 1-bit residual.

K5 = K4 + 2-bit Lloyd-Max. K6 = K5 + 1-bit scalar on the K5 residual.
Tests whether we can extend the smooth Pareto curve beyond 5.0 bpw.
"""
...
def run_experiment(data_dir, device, ext, ghd, tcp, tcpi, qtf, cbs):
    ...
    for ei in range(n_experts):
        ...
        # 4-tier: K3 → K4 → K5 → K6 (each upgrade costs 1 bpw per tile)
        # K3→K4: 1 bpw, K4→K5: 1 bpw (2-bit lloyd), K5→K6: 1 bpw (1-bit scalar)
        upgrades = []
        ...
        for target_bpw_10 in range(30, 66):  # 3.0 to 6.5
        ...
        del w_reg, qk3, qk4, r4, lloyd, recon_k5, r5, scalar_1bit, recon_k6
        del tmse_k3, tmse_k4, tmse_k5, tmse_k6
        torch.cuda.empty_cache()
    
    print(f"\n  Extended Pareto frontier (3.0-6.5 bpw, 0.1-bit steps):", flush=True)
    pareto = []
...
def main():
    ...
    ap.add_argument("--out", default="/tmp/poc_residual/results_v16.json")
    args = ap.parse_args()
