#!/usr/bin/env python3
"""PoC v26b: Cross-layer codebook sharing (normalized).

Codebooks are stored as normalized levels (divided by per-cluster sigma).
At apply time, multiply by target's per-cluster sigma.
This makes codebooks transferable across layers/projections.
"""
...
def cluster_by_sigma(tiles, n_clusters, device):
    ...
    return cluster_id, sigmas

def train_normalized_codebooks(tiles, cluster_id, n_bits, n_clusters, device):
    """Train codebooks, return normalized (sigma=1) levels."""
    n_levels = 2 ** n_bits
    codebooks = torch.zeros(n_clusters, n_levels, device=device)
    for cid in range(n_clusters):
        ...
        # Normalize by sigma
        normalized = cluster_flat / sigma
        levels = torch.linspace(-3, 3, n_levels, device=device)
        for _ in range(12):
            d = (normalized.unsqueeze(1) - levels.unsqueeze(0)).abs()
            assign = d.argmin(dim=1)
            new_levels = levels.clone()
            for i in range(n_levels):
                m = assign == i
                if m.sum() > 0: new_levels[i] = normalized[m].mean()
            if (new_levels - levels).abs().max() < 1e-10: break
            levels = new_levels
        codebooks[cid] = levels
    return codebooks

def apply_normalized_codebooks(tiles, cluster_id, codebooks, n_clusters):
    """Apply normalized codebooks, scaled by per-cluster sigma."""
    n_tiles = tiles.shape[0]
    sigmas = tiles.std(dim=1).clamp(min=1e-12)
    ...
    for cid in range(n_clusters):
        ...
        cluster_sigma = sigmas[mask].mean()
        levels = codebooks[cid] * cluster_sigma
...
def run_experiment(data_dir, device, ext, ghd, tcp, tcpi, qtf, cbs):
    ...
    k, n = list(all_data.values())[0].shape[1:]

    # Train codebooks on first expert of each (layer, proj)
    trained = {}  # (layer, proj) -> (codebooks tensor, cluster_id)
    for (layer, proj), experts in sorted(all_data.items()):
        ...
        cid, _ = cluster_by_sigma(tiles, n_clusters, device)
        cbs_norm = train_normalized_codebooks(tiles, cid, n_bits, n_clusters, device)
        trained[(layer, proj)] = cbs_norm
        del w_reg, qk4, r4, tiles
        torch.cuda.empty_cache()

    # Test: own vs cross codebooks (all on first expert)
    print(f"\n  Cross-layer codebook sharing (normalized, 2-bit LM, 64 clusters):", flush=True)
    ...
    for (layer_a, proj_a), experts in sorted(all_data.items()):
        ...
        r4 = w_reg - qk4
        tiles = get_tiles(r4, ek, en)
        cid_a, _ = cluster_by_sigma(tiles, n_clusters, device)

        # Own codebooks
        recon_own = qk4 + apply_normalized_codebooks(tiles, cid_a, trained[(layer_a, proj_a)], n_clusters).reshape(ek, en)
        mse_own = (w_reg - recon_own).pow(2).mean().item()
        own_mses[(layer_a, proj_a)] = mse_own

        # Cross codebooks: try all trained codebooks
        for (layer_t, proj_t), cbs_t in sorted(trained.items()):
            recon_cross = qk4 + apply_normalized_codebooks(tiles, cid_a, cbs_t, n_clusters).reshape(ek, en)
            ...
            cross_results.append({
                "train": f"L{layer_t}_{proj_t}", "apply": f"L{layer_a}_{proj_a}",
                "mse": mse_cross, "ratio_vs_own": ratio
            })
    ...
    results["cross_results"] = cross_results
    return results
...
def main():
    ...
    ap.add_argument("--out", default="/tmp/poc_residual/results_v26.json")
    args = ap.parse_args()
